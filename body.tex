...
pinhole projector. Since the projector does not have fast illumination control, we primarily used the LSST camera main shutter instead of any flashing of the light source (e.g., as we did with the LEDs of the CCOB Wide Beam). One
...
\begin{figure}[H]
\begin{centering}
\includegraphics[width=0.7\textwidth]{figures/baselineCharacterization/BRIGHT PIXELS_13550_E1071_diff.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/baselineCharacterization/DIVISIDERO TEARING_13550_E1071_diff.png}
...
\begin{figure}[H]
    ...
    \caption{Comparison of dark current measurements for initial and final run 7 measurements. A marked decrease in dark current is present in rafts local to the autochanger light leak (see section \ref{light-leak-mitigation-with-shrouding-the-camera-body})}
...
\begin{itemize}
    ...
    \item E756 (dp80, gain stability @ 50k e-), unprocessed
    \item E1362 (dp80, 10k e-), unprocessed
\end{itemize}
...
Figure \ref{fig:gaintempE1496} shows the relationship between relative gain and the REB temperature, with color coding by the speed of temperature change, along with its fit. A reference line from the past result is overlaid with an arbitrary vertical offset. Clearly, the gain-temp relationship is steeper than the previous result.
