...
Define Run 5, Run 6a, Run 6b, Run 7
...
We found that the diffuser greatly reduced the `weather' (Fig.~\ref{fig:weather}) and eliminated the CMB pattern and more uniformly illuminated the focal plane (Fig.~\ref{fig:roll-off}), with a penalty of decreaseing the overall illumination by roughly 35\% even though we fully opened the f-stop.
...
\includegraphics[width=0.7\textwidth]{figures/baselineCharacterization/13550_E1071_SCTI_EF_43_inset.png}
	\caption{Serial CTI comparison by raft for Run 7 (E1017) and Run 6 (13550)\label{fig:serial-cti}}
\end{centering}
\end{figure}
...
\caption{Bright pxiel comparison by raft for Run 7 (E1017) and Run 6 (13550)}
\label{fig:bright}
\end{centering}
\end{figure}
...
\caption{A comparison of Run 6 and Run 7 amplifier measurement in the $a_{00}$ coefficient, separated by sensor type. For both sensor types, the $a_{00}$ coefficient is very consistent.}
\end{centering}
\end{figure}
...
\caption{A comparison of Run 6 and Run 7 amplifier differences in the $a_{00}$ coefficient, separated by sensor type. For both sensor types, the $a_{00}$ coefficient is very consistent.}
\end{centering}
\end{figure}
...
\caption{A comparison of Run 6 and Run 7 amplifier measurements in brighter-fatter x-correlation, separated by sensor type. For both sensor types, brighter-fatter x-correlation is very consistent.}
\end{centering}
\end{figure}
...
\caption{A comparison of Run 6 and Run 7 amplifier measurements in brighter-fatter y-correlation, separated by sensor type. For both sensor types, brighter-fatter y-correlation is very consistent.}
\end{centering}
\end{figure}
...
\caption{A comparison of Run 6 and Run 7 amplifier differences in brighter-fatter x-correlation, separated by sensor type. For both sensor types, brighter-fatter x-correlation is very consistent.}
\end{centering}
\end{figure}
...
\caption{A comparison of Run 6 and Run 7 amplifier differences in brighter-fatter y-correlation, separated by sensor type. For both sensor types, brighter-fatter y-correlation is very consistent.}
\end{centering}
\end{figure}
...
Differences in row-means variance between runs are evident, and are distinctly different for different detector types. The difference between runs is more significant for ITL sensors, \textasciitilde9\% smaller on average in Run 7. For E2V sensors, the effect is \textasciitilde3\% smaller in Run 7. This indicates that either row-by-row correlated noise or row-by-row gain change is less in Run 7b. Since we didn't change the sequencer file, the most natural explanation is the row-by-row correlated noise. But further investigation is needed.
...
Divisadero tearing (or Rabbit ear) is manifested as signal variations near amplifier boundaries, connected features that are often jagged \cite{2020arXiv200209439J,2024SPIE13103E..0WU}. These variations are on the order of \textasciitilde1\% relative to the flat field signal. To quantify divisadero tearing in a given column, we measure the column signal, and compare it to the mean column signal from flat fields.
...
Divisadero tearing is broadly consistent between Run 6 and Run 7, with both sensor types demonstrating lower Divisadero tearing in Run 7. Taking amplifier differences, E2V sensors show a weaker Divisadero signal in Run 7 by 0.1\%, while ITL sensors demonstrate a weaker Divisadero signal in Run 7 by 0.05\% (see figure \ref{fig:divisidero_diff_baseline}).
...
\begin{figure}[H]
\begin{centering}
\includegraphics[width=0.7\textwidth]{figures/baselineCharacterization/13550_E1071_DARK_PIXELS_inset.png}
...
\begin{subfigure}{0.5\textwidth}
  \centering
  \includegraphics[width=1\textwidth]{figures/baselineCharacterization/persistence_plot_LSSTCam_R12_S21_u_lsstccs_eo_persistence_13550_w_2023_41_20231117T001459Z.png}
\end{subfigure}
\caption{Comparison of persistence profiles for R12\_S21 between (left) Run 7 (E1071) and (right) Run 6 (13550).  The decay time constants are similar but the initial persistence level is greater in Run 7.  The asymptotic levels are also different.}
...
Linearity turnoff {[}e-{]}         & Val               & \multicolumn{1}{l|}{112410.98} & 112162.66 & \multicolumn{1}{l|}{105960.37} & 106002.95  \\ \hline
PTC turnoff {[}e-{]}               & Val               & \multicolumn{1}{l|}{90422.94}  & 89697.03  & \multicolumn{1}{l|}{78209.44}  & 77913.08   \\ \hline
...
Dark defects {[}count{]}           & Val               & \multicolumn{1}{l|}{1.6890E+3} & 1.7090E+3 & \multicolumn{1}{l|}{1.5130E+3} & 1.5070E+3  \\ \hline
...
\item {\bf Phase overlap during parallel transfer for e2v}: e2v sensors feature four parallel phases. To improve the uniformity of the full well across a sensor, overlapping two phases during each time slice of the parallel transfer was introduced. However, this overlap is known to cause trailing persistence, as reported in the Davis Report. We conducted several runs using both intermediate overlapping and non-overlapping sequencers. By optimizing the operating voltages to avoid charge trapping, the trailing persistence is no longer a concern.
\end{itemize}
...
\item
  ...
  in the subsequent 20--50 rows (see left-hand plot in Fig.~\ref{fig:clear_e2v}).
...
charges, a standard readout of \textgreater\textasciitilde{} 2000 rows 
does remove them.

The localization of these uncleared electrons in the first row of the
...
register field as the rows are moved to it. The constraint being that
...
right: V29, Nop, and NopSf), a zoom on the first row of an ITL or e2v
...
residual signal is visible in the first \textasciitilde20 rows. The
...
For a description of each quantity within this section and its acquisition process, refer to section \ref{reverification}.
...
\begin{figure}[H]
    ...
    \caption{Comparison of dark current measurements for initial and final run 7 measurements. A marked decrease in dark current is present in rafts local to the autochanger light leak (see section \ref{successful-autochanger-light-leaks-masking})}
...
\begin{figure}
    ...
    \caption{Comparison of linearity turnoff measurements from the initial and final run 7 measurements. The E2V sensors show a notable decrease in linearity turnoff, while ITL sensors stay relatively the same.}
...
\begin{figure}
    ...
    \caption{Comparison of PTC turnoff measurements from the initial and final run 7 measurements. The E2V sensors show a notable decrease in PTC turnoff, while ITL sensors stay relatively the same.}
    \label{fig:finalChar-PTCTurnoff-5x5}
\end{figure}

For E2V sensors we find the reduction in full-well to be significant, \textasciitilde20 - 25\% depending on the full-well metric used (see figure \ref{fig:finalChar-fullWell}).
...
E2V amplifiers show increased gain in final run 7 conditions by an average of \textasciitilde0.03 e-/ADU.
...
\paragraph{Brighter fatter $a_{00}$ coefficien}\label{final-brighter-fatter-a00-coefficient}

The relative strength of the brighter-fatter effect, quantified by $a_{00}$ following \cite{2019A&A...629A..36A}'s, is modified in the final run 7 operating conditions by the lower parallel swing for E2V sensors. We observe an extremely high consistency for ITL sensors.
...
The change in the $a_{00}$ value for E2V sensors is illustrated in figure \ref{fig:finalChar-PTC_A00_E2VComp}, showing a \textasciitilde12\% increase in the strength of the brighter fatter effect for E2V sensors due to the lower parallel swing.
...
Dark current {[}e-/pix/s{]}       &                                & \multicolumn{1}{l|}{2.4783E-02}              &       2.3188E-02      & \multicolumn{1}{l|}{2.1217E-02}              &       2.0734E-02      \\ \hline
Bright defects {[}count{]}        &                                & \multicolumn{1}{l|}{0}              &   0          & \multicolumn{1}{l|}{0}              &          0   \\ \hline
Linearity turnoff {[}e-{]}        &                                & \multicolumn{1}{l|}{112162.66}              &        90323.14     & \multicolumn{1}{l|}{106002.95}              &      105896.708       \\ \hline
PTC turnoff {[}e-{]}              &                                & \multicolumn{1}{l|}{8.9697E+04}              &      6.7750E+04       & \multicolumn{1}{l|}{7.7913E+04}              &        7.7925E+04     \\ \hline
PTC Gain {[}e- / ADU{]}           &                                & \multicolumn{1}{l|}{1.4812}              &      1.5107       & \multicolumn{1}{l|}{1.6761}              &       1.6752      \\ \hline
PTC $a_{00}$ [$\frac{1}{pix^2}$]  &                                & \multicolumn{1}{l|}{3.0863E-06}              &       3.4899E-06      & \multicolumn{1}{l|}{1.7031E-06}              &        1.7009E-06     \\ \hline
BF x-correlation                  &                                & \multicolumn{1}{l|}{5.1693E-01}              &     5.1022E-01        & \multicolumn{1}{l|}{7.5212E-01}              &       7.3648E-01      \\ \hline
BF y-correlation                  &                                & \multicolumn{1}{l|}{1.7077E-01}              &       1.6740E-01      & \multicolumn{1}{l|}{2.8695E-01}              &        2.8439E-01     \\ \hline
Row-means variance                &                                & \multicolumn{1}{l|}{8.8367E-01}              &      8.6809E-01       & \multicolumn{1}{l|}{9.4664E-01}              &        9.4633E-01     \\ \hline
Dark defects {[}count{]}          &                                & \multicolumn{1}{l|}{1709}              &       1968      & \multicolumn{1}{l|}{1507}              &       1508      \\ \hline
Divisadero tearing maximum {[}\%{]} &     None                     & \multicolumn{1}{l|}{6.2622E-03}              &       2.4599E-03      & \multicolumn{1}{l|}{2.7348E-03}              &       2.7414E-03      \\ \hline
Persistence {[}ADU{]}             &       None                         & \multicolumn{1}{l|}{5.6673E+00}              &       4.0181E-01      & \multicolumn{1}{l|}{4.8018E-01}              &       3.2639E-01      \\ \hline
\end{tabular}
...
\begin{table}[!ht]
    ...
    \hline
        ...
        R01_S01_C00  & Dead Amp & Dead & Dead & OK & OK & OK & OK & OK & OK \\ \hline
        R01_S02_C07  & Hi Noise & OK & 27e & 22e & 20e & 21e & 15e & 14e & 14e \\ \hline
        R01_S11_C00  & Hi Noise & OK & 24e & OK & OK & 12e & OK & OK & OK \\ \hline
        R03_S11_C00  & Dead Amp & OK & NA & OK & OK & Dead & Dead & Dead & Dead \\ \hline
        R10_S00_C00  & Dead Amp & Dead & NA & OK & OK & OK & OK & OK & OK \\ \hline
        R30_S00_C10  & Dead Amp & Dead & Dead & Dead & Dead & Dead & Dead & Dead & Dead \\ \hline
        R41_S11_C14  & Hi Noise & OK & NA & 36e & OK & OK & OK & OK & OK \\ \hline
        R41_S21_C02  & Hi Noise & OK & NA & OK & 108e & 96e & 85e & 110e & 115e \\ \hline
        R43_S02_C03  & Hi Noise & 18e & NA & 18e & 18e & 18e & 17e & 18e & 17e \\ \hline
        R43_S20_C14  & Hi Noise & OK & NA & OK & OK & 69e & 145e & OK & OK \\ \hline
...
R01_S01_C00  & SometimesDead & Usually OK, turns Dead in E3577, previously seen as Dead \\ \hline
R01_S02_C07  & OK & noise fluctuates sometimes over $18e^-$ consistent with previous behavior \\ \hline
R01_S11_C00  & OK & noise fluctuates sometimes over $18e^-$ consistent with previous behavior \\ \hline
R03_S01_C05  & SometimesHiNoise & previously OK,  High Noise in E3577 for first time, NEW bad amp \\ \hline
R03_S11_C00  & SometimesDead & Usually OK, turns Dead in E3577, previously seen as Dead \\ \hline
R30_S00_C10  & Dead & always Dead \\ \hline
R41_S21_C02  & HiNoise & always Hi Noise \\ \hline
R43_S20_C14  & HiNoise & always Hi Noise \\ \hline

    \end{tabular}
    \caption{Table of potentially non-functioning Science Raft amplifiers, from Runs 6a, 6b, and 7. \label{tab:badamps}}
\end{table}
...
\makecell{\textbf{Noise} \\ \textbf{(??)}} \\
\hline
\hline
\endhead
\multicolumn{7}{|c|}{\textbf{Noise Study Configurations}} \\
\hline
50x50   & 50  & 1 & 1 & n/a       &  9.28 & \\ % gds_noise_01.cfg
50x50   & 50  & 2 & 1 & aligned   &  9.27 & \\ % gds_noise_02.cfg
50x50   & 50  & 2 & 1 & unaligned &  9.26 & \\ % gds_noise_03.cfg
50x50   & 50  & 4 & 4 & aligned   &  9.26 & \\ % gds_noise_04.cfg
50x50   & 50  & 4 & 4 & unaligned &  9.26 & \\ % gds_noise_05.cfg
50x50   & 50  & 8 & 4 & aligned   &  9.23 & \\ % gds_noise_06.cfg
50x50   & 50  & 8 & 4 & unaligned &  9.23 & \\ % gds_noise_07.cfg
\hline
\multicolumn{7}{|c|}{\textbf{Nominal Configurations}} \\
\hline
50x50   & 50  & 8 & 4 & aligned   &  9.22 & \\ % gds_nom_aligned.cfg
50x50   & 50  & 8 & 4 & unaligned &  9.23 & \\ % gds_nom_unaligned.cfg
\hline
\multicolumn{7}{|c|}{\textbf{ROI Study Configurations}} \\
\hline
400x400 & 200 & 1 & 1 & n/a       &  1.67 & \\ % gds_roi_01.cfg
400x400 &  50 & 1 & 1 & n/a       &  2.23 & \\ % gds_roi_02.cfg
400x400 &   5 & 1 & 1 & n/a       &  2.48 & \\ % gds_roi_03.cfg
10x10   &  50 & 1 & 1 & n/a       & 11.80 & \\ % gds_roi_04.cfg
400x400 &  50 & 1 & 1 & SplitROI  &  2.23 & \\ % gds_roi_05.cfg
\hline
\end{longtable}

\subsection*{Noise Investigation (take 20 images for each, 15 seconds each)}

\subsubsection*{Start with a single GREB}
\begin{itemize}
...
\subsection*{ROI reconstruction (take 20 images for each, 15 seconds each)}
\subsubsection*{Unsplit ROI different exposures and sizes}
\begin{itemize}
    \item 200 ms
    \begin{verbatim}
    gds_roi_01.cfg
...
\begin{itemize}
    ...
    \item Global gain change could be due to the artifacts/setup. Potentially, the system could have the complicated behavior with respect to the REB temperature. No conclusive statement is made.
    \item Local amp-by-amp gain change is minimal $10^{-4}$ over 6 hours if the focal plane temperature is maintained at the same.
    \item Further analysis is needed in understanding the gain change in the beginning of Run is needed.
\end{itemize}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.125}}
...
Parameter
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
dp80 (Run 7 E2V voltage)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Run 7 ITL voltage (cloned from Run 7)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
dp93 (Run 5)
\end{minipage} \\
\midrule\noalign{}
...
\multirow{3}{*}{PTC}            & E1881  &       & Dense red PTC with no dark interleaves, First V30 sequencers            \\ \cline{2-4} 
                                & E1886  &       & Dense red PTC with dark interleaves, V30 sequencers                     \\ \cline{2-4} 
                                & E2237  &       & Randomized flat pairs, no dark interleave, after full CCS system reboot \\ \hline
\multirow{3}{*}{Gain Stability} & E2008  &       &                                                                         \\ \cline{2-4} 
                                & E1968  &       &                                                                         \\ \cline{2-4} 
                                & E1955  &       & 6h Stability run 10k 750 nm V30, ,                                      \\ \hline
...
\hline
Run ID & Links & Notes \\ \hline
E1886 & & Red LED dense. Dark interleaving between flat pairs \\ \hline
...
E2330 & & Short dark sequence, filter changes in headers through OCS \\ \hline
...
\hline
Run ID & Links & Notes \\ \hline
E2015 & & 10 flats at 10 ke$^-$ followed by 10$\times$15\,s darks \\ \hline
