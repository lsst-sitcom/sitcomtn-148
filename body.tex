...
\textasciitilde{}0.04 e$^-$/s/pixel was found to be associated with the 24\,V Clean of
the AC.
...
maintained its performance charateristics between Run 6 and Run 7.
...
For comparisons between Cerro Pachon EO runs and the final SLAC IR2 equivalents, the following runs are used (see Table \ref{runTable}).
...
Run 7. Both sensor types show extremely low CTI on the order of $10^{-3}$ \%,
and differ on the order of \textasciitilde$2 \times 10^{-5}$ \% for e2v sensors, and
by \textasciitilde$4 \times 10^{-6}$  \% for ITL sensors.
...
Run 7. Both sensor types are found to have extremely low CTI on the order of $10^{-5}$ \%,
and differ on the order of \textasciitilde2E-7 \% for e2v sensors, and
by \textasciitilde$7 \times 10^{-6}$ \% for ITL sensors.
...
Run 6. A possible reason for this could be improved shrouding
on the camera in the Level 3 white room than in the IR2 clean room SLAC.
...
sensors is very consistent between runs, and much weaker than for e2v.
...
In Run 7 the maximum divisadero tearing amplitude for e2v CCDs is \textasciitilde0.3\% higher in run 7. In ITL sensors, maximum divisadero tearing is \textasciitilde0.1\% higher in Run 6.
...
defined as a $-$20\% deviation from flatness.
...
near the edges of the amplifier segments.
...
The correspondence between {\it vampire pixels} and spot-like phosphorescence is briefly laid out here, for three prominent cases. R01_S00_C13-14  R03_S10_C15 R20_S20_C13
